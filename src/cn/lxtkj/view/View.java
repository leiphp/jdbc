	...
	public static void main(String[] args) {
		...
		while(scan.hasNext()){
			...
			if(OPERATION_EXIT.equals(in.toUpperCase())||OPERATION_EXIT.substring(0, 1).equals(in.toUpperCase())) {
				...
				//查询单个女神
				System.out.println("请输入要查询的女神的 [编号]");
				Integer id=scan.nextInt();
				
				try {
					Goddess g2=action.get(id);
					System.out.println(g2.getId()+",姓名："+g2.getUser_name());
					
				} catch (SQLException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					System.out.println("查询女神失败");
				}									
			}else if(OPERATION_DELETE.equals(in.toUpperCase())||OPERATION_DELETE.substring(0, 1).equals(in.toUpperCase())) {				
				//删除单个女神
				System.out.println("请输入要删除的女神的 [编号]");
				Integer id=scan.nextInt();
				...
				if(1==step) {
					...
					try {
						...
						e.printStackTrace();
						System.out.println("请输入的格式有误，请重新输入");
						step=3;
					}
				...
				System.out.println("您输入的值为："+in+step);
			}
